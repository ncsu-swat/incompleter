...
from main.utils.unmocker import unmock_code_snippet, prepare_context
...
class Moxecutor():
    def __init__(self, snippet_path: str, is_cov: bool, type_dict = None, max_iter = MAX_ITER) -> None:
        try:
            ...
            while _iter < self.MAX_ITER: # Fix-point loop
                try:
                    ...
                    if self.is_cov:
                        ...
                        out, err, cov_summary, stmt_cov, br_cov = self.snippet.compute_timed_latest_coverage()
                        # print('\n\nOutput:\n{}'.format(out))
                        ...
                        if cov_summary is not None:
                            for iter_idx in range(_iter, self.MAX_ITER):
                                if iter_idx not in coverage_report.keys():
                                    coverage_report[iter_idx] = { 'cov_summary': None, 'stmt': None, 'br': None }
                                coverage_report[iter_idx]['cov_summary'] = cov_summary
    ...
    def unmocked_snippet_report(self, _iter, coverage_report, executability_report, action_progress_report, action_iteration_report):
        if (not Session.lextp) or (Session.lextp and MAX_ITER-1 in coverage_report.keys()):
            # _iter = _iter + 1
            self.snippet.history[-1], deductions_tally = unmock_code_snippet(self.snippet, coverage_report[self.MAX_ITER-1])
            
            if self.type_dict is not None and self.type_dict['variable'] in self.snippet.mocked_values.keys():
                var = self.type_dict['variable']
                tbd = self.snippet.mocked_values[self.type_dict['variable']]
                _type = self.type_dict['type']
                context = prepare_context(self.snippet.history[-1].strip(), self.snippet, tbd)
                self.append_data_row('nn_dataset.tsv', self.snippet_name.replace('snippet_', '').replace('.py.orig', ''), '\n'.join(self.snippet.history[0].split('\n')[1:]).strip(), self.snippet.history[-1].strip(), context, var, tbd, _type)
            elif self.type_dict is not None:
                var = self.type_dict['variable']
                _type = self.type_dict['type']
                self.append_data_row('nn_dataset.tsv', self.snippet_name.replace('snippet_', '').replace('.py.orig', ''), '\n'.join(self.snippet.history[0].split('\n')[1:]).strip(), self.snippet.history[-1].strip(), '', var, '', _type)
    ...
    def append_data_row(self, data_file_name, snippet_no, original_snippet, unmocked_snippet, context, var, tbd, type):
        data_file_path = os.path.join(DATA_DIR, 'new_all', 'nn_dataset', data_file_name)
        
        if not os.path.exists(data_file_path):
            with open(data_file_path, 'w+') as data_file:
                csv.writer(data_file, delimiter='\t').writerow(['snippet_no', 'original_snippet', 'mocked_snippet', 'context', 'var', 'tbd', 'type'])

        with open(data_file_path, 'a+') as data_file:
            csv.writer(data_file, delimiter='\t').writerow([str(snippet_no), repr(original_snippet), repr(unmocked_snippet), repr(context), var, tbd, type])