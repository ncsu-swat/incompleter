    ...
    def build_shift_dict(shift):
        ...
        def shift_letters(shift, text):
            for i in range(len(text)):
                if i < shift:
                    shifted_dict[text[i]] = text[26 - shift + i]
                else:
                    shifted_dict[text[i]] = text[i - shift]

        shift_letters(shift, string.ascii_lowercase)
        shift_letters(shift, string.ascii_uppercase)
        return shifted_dict
...
class CiphertextMessage(Message):
    ...
    def decrypt_message(self):
        ...
        '''
        for i in range(26):
            decrypted = Message.apply_shift(self.message , i)
            for word in decrypted :
                if word in Message.valid_words :
                    return (i , decrypted)
